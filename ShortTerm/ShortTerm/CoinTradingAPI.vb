...
Public Class CoinTradingAPI
    ...
    End Function
    Function GetMarketNameFromID(marketID As Integer) As String
        Dim sqlCommand As New SqlCommand("Bittrex.Markets_GetMarketNameFromID", Me.SqlConnection)
        sqlCommand.CommandType = CommandType.StoredProcedure
        AddSQLParameter(sqlCommand, "MarketID", SqlDbType.Int, marketID)
        AddSQLOutputParameter(sqlCommand, "MarketName", SqlDbType.VarChar, 10)

        sqlCommand.ExecuteNonQuery()

        Return GetSQLParameterStringValue(sqlCommand, "MarketName")
    End Function
    ...
    Function DownloadDayTicks(ByVal MarketID As Integer) As List(Of Tick)
        Dim sqlCommand As New SqlCommand("Bittrex.DayTicks_Download", Me.SqlConnection)
        sqlCommand.CommandType = CommandType.StoredProcedure
        AddSQLParameter(sqlCommand, "MarketID", SqlDbType.Int, MarketID)

        Dim dayTicks As New List(Of Tick)

        For Each dataRow As DataRow In DownloadSqlDataset(sqlCommand).Tables(0).Rows
            Dim tick As Tick = New Tick()
            tick.BV = CDbl(dataRow.Item("BaseVolume"))
            tick.C = CDbl(dataRow.Item("Close"))
            tick.H = CDbl(dataRow.Item("High"))
            tick.L = CDbl(dataRow.Item("Low"))
            tick.O = CDbl(dataRow.Item("Open"))
            tick.T = CDate(dataRow.Item("TimeStamp"))
            tick.V = CDbl(dataRow.Item("Volume"))
    ...
    Function DownloadDayTicks(ByVal MarketID As Integer, ByVal StartDate As String) As List(Of Tick)
        Dim sqlCommand As New SqlCommand("Bittrex.DayTicks_DownloadFrom", Me.SqlConnection)
        sqlCommand.CommandType = CommandType.StoredProcedure
        AddSQLParameter(sqlCommand, "MarketID", SqlDbType.Int, MarketID)
        AddSQLParameter(sqlCommand, "Start", SqlDbType.Date, StartDate)
    ...
    Function DownloadHourTicks(ByVal MarketID As Integer) As List(Of Tick)
        Dim sqlCommand As New SqlCommand("Bittrex.HourTicks_Download", Me.SqlConnection)
        sqlCommand.CommandType = CommandType.StoredProcedure
        AddSQLParameter(sqlCommand, "MarketID", SqlDbType.Int, MarketID)

        Dim hourticks As New List(Of Tick)
    ...
    Function DownloadHourTicks(ByVal MarketID As Integer, ByVal StartDate As String) As List(Of Tick)
        Dim sqlCommand As New SqlCommand("Bittrex.HourTicks_DownloadFrom", Me.SqlConnection)
        sqlCommand.CommandType = CommandType.StoredProcedure
        AddSQLParameter(sqlCommand, "MarketID", SqlDbType.Int, MarketID)
        AddSQLParameter(sqlCommand, "Start", SqlDbType.Date, StartDate)
